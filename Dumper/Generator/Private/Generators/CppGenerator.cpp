...
{})", Settings::CppGenerator::GetImageBaseFuncBody);
	...
	/* Generic, header-only math utilities (concept-based; works with SDK::FVector on both UE4 float and UE5 double). */
	BasicHpp << R"(
namespace Math
{

template<typename T> concept Vec2        = requires(T v) { v.X; v.Y; };
template<typename T> concept Vec3        = requires(T v) { v.X; v.Y; v.Z; };
template<typename T> concept Vec4        = requires(T v) { v.X; v.Y; v.Z; v.W; };
template<typename T> concept Quat        = Vec4<T>;
template<typename T> concept Rotator     = requires(T r) { r.Pitch; r.Yaw; r.Roll; };
template<typename T> concept LinearColor = requires(T c) { c.R; c.G; c.B; c.A; };

template<typename V> using scalar_t         = std::remove_cvref_t<decltype(std::declval<V>().X)>;
template<typename R> using rotator_scalar_t = std::remove_cvref_t<decltype(std::declval<R>().Pitch)>;
...
template<typename T> constexpr T Clamp  (T v, T lo, T hi)      noexcept { return v < lo ? lo : (v > hi ? hi : v); }
...
template<Vec3 V> constexpr scalar_t<V> Dot        (const V& a, const V& b) noexcept { return a.X*b.X + a.Y*b.Y + a.Z*b.Z; }
template<Vec3 V> constexpr scalar_t<V> SizeSquared(const V& v)             noexcept { return v.X*v.X + v.Y*v.Y + v.Z*v.Z; }
template<Vec3 V> scalar_t<V>           Size        (const V& v)             noexcept { return std::sqrt(SizeSquared(v)); }
template<Vec3 V> constexpr scalar_t<V> DistSq      (const V& a, const V& b) noexcept
{
    using T = scalar_t<V>;
    const T dx = b.X-a.X, dy = b.Y-a.Y, dz = b.Z-a.Z;
    return dx*dx + dy*dy + dz*dz;
}
template<Vec3 V> scalar_t<V> Dist(const V& a, const V& b) noexcept { return std::sqrt(DistSq(a, b)); }
template<Vec3 V> constexpr V Cross(const V& a, const V& b) noexcept
{
    return V{ a.Y*b.Z - a.Z*b.Y, a.Z*b.X - a.X*b.Z, a.X*b.Y - a.Y*b.X };
}
template<Vec3 V> V GetNormalized(const V& v) noexcept
{
    using T = scalar_t<V>;
    const T sz = Size(v);
    return sz > T(SmallNum) ? V{ v.X/sz, v.Y/sz, v.Z/sz } : V{};
}
template<Vec3 V> V Reflect(const V& v, const V& n) noexcept
{
    using T = scalar_t<V>;
    const T d = Dot(v, n);
    return V{ v.X - T(2)*d*n.X, v.Y - T(2)*d*n.Y, v.Z - T(2)*d*n.Z };
}
template<Vec3 V> V ProjectOnTo(const V& v, const V& onto) noexcept
{
    using T = scalar_t<V>;
...
template<Vec3 V> constexpr V ComponentMin(const V& a, const V& b) noexcept
{
    return V{ a.X<b.X?a.X:b.X, a.Y<b.Y?a.Y:b.Y, a.Z<b.Z?a.Z:b.Z };
}
template<Vec3 V> constexpr V ComponentMax(const V& a, const V& b) noexcept
{
    return V{ a.X>b.X?a.X:b.X, a.Y>b.Y?a.Y:b.Y, a.Z>b.Z?a.Z:b.Z };
}
template<Vec3 V> constexpr V Abs(const V& v) noexcept
{
    using T = scalar_t<V>;
    return V{ v.X<T(0)?-v.X:v.X, v.Y<T(0)?-v.Y:v.Y, v.Z<T(0)?-v.Z:v.Z };
}
template<Vec3 V> scalar_t<V> AngleBetween(const V& a, const V& b) noexcept
{
    using T = scalar_t<V>;
    const T sa = Size(a), sb = Size(b);
    if (sa < T(SmallNum) || sb < T(SmallNum)) return T(0);
    return ToDegrees(std::acos(Clamp<T>(Dot(a,b)/(sa*sb), T(-1), T(1))));
}
template<Vec3 V> scalar_t<V> AngleBetweenNormals(const V& a, const V& b) noexcept
{
    using T = scalar_t<V>;
    return ToDegrees(std::acos(Clamp<T>(Dot(a,b), T(-1), T(1))));
}
template<Vec3 V> constexpr V Lerp(const V& a, const V& b, scalar_t<V> t) noexcept
{
    return V{ Lerp(a.X,b.X,t), Lerp(a.Y,b.Y,t), Lerp(a.Z,b.Z,t) };
}
/* Rodrigues — rotate v around axis (need not be unit) by angleDeg degrees. */
template<Vec3 V> V RotateAngleAxis(const V& v, scalar_t<V> angleDeg, const V& axis) noexcept
{
    using T = scalar_t<V>;
    ...
    const T d = Dot(n, v);
    const T omc = T(1) - c;
    return V{ v.X*c + cr.X*s + n.X*d*omc, v.Y*c + cr.Y*s + n.Y*d*omc, v.Z*c + cr.Z*s + n.Z*d*omc };
}

template<Quat Q> constexpr scalar_t<Q> QuatLengthSq(const Q& q) noexcept { return q.X*q.X+q.Y*q.Y+q.Z*q.Z+q.W*q.W; }
template<Quat Q> scalar_t<Q>           QuatLength   (const Q& q) noexcept { return std::sqrt(QuatLengthSq(q)); }
template<Quat Q> Q QuatNormalize(const Q& q) noexcept
{
    using T = scalar_t<Q>;
...
template<Quat Q> constexpr Q QuatInverse (const Q& q) noexcept { return Q{ -q.X, -q.Y, -q.Z, q.W }; }
/* Hamilton product: a * b */
template<Quat Q> constexpr Q QuatMultiply(const Q& a, const Q& b) noexcept
{
...
/* Rotate a Vec3 by a quaternion. Scalar type is deduced from V (not Q). */
template<Quat Q, Vec3 V> V QuatRotateVector(const Q& q, const V& v) noexcept
{
    using T = scalar_t<V>;
    ...
    return V{ v.X+T(2)*outer.X, v.Y+T(2)*outer.Y, v.Z+T(2)*outer.Z };
}
template<Quat Q> Q QuatSlerp(const Q& a, const Q& b, scalar_t<Q> t) noexcept
{
    using T = scalar_t<Q>;
    ...
    if (d > T(1)-T(KindaSmall))
        return QuatNormalize(Q{ Lerp<T>(a.X,B.X,t), Lerp<T>(a.Y,B.Y,t),
                                Lerp<T>(a.Z,B.Z,t), Lerp<T>(a.W,B.W,t) });
...
template<Quat Q, Vec3 V> Q QuatFromAxisAngle(const V& axis, scalar_t<V> deg) noexcept
{
    using T = scalar_t<V>;
...
template<Vec3 V, Rotator R> V RotatorToVector(const R& r) noexcept
{
    using T  = scalar_t<V>;
    using TR = rotator_scalar_t<R>;
    const T cp = static_cast<T>(std::cos(ToRadians<TR>(r.Pitch)));
    const T sp = static_cast<T>(std::sin(ToRadians<TR>(r.Pitch)));
    const T cy = static_cast<T>(std::cos(ToRadians<TR>(r.Yaw)));
    const T sy = static_cast<T>(std::sin(ToRadians<TR>(r.Yaw)));
    return V{ cp*cy, cp*sy, sp };
}
template<Rotator R, Vec3 V> R VectorToRotator(const V& v) noexcept
{
    using TR = rotator_scalar_t<R>;
    using TV = scalar_t<V>;
    const TV sz    = Size(v);
    const TR yaw   = static_cast<TR>(ToDegrees<TV>(std::atan2(v.Y, v.X)));
    const TR pitch = sz > TV(SmallNum) ? static_cast<TR>(ToDegrees<TV>(std::asin(v.Z/sz))) : TR(0);
    return R{ pitch, yaw, TR(0) };
}
/* Unreal rotator -> quaternion (ZYX Tait-Bryan, half-angle form). */
template<Quat Q, Rotator R> Q RotatorToQuat(const R& r) noexcept
{
    using T  = scalar_t<Q>;
    using TR = rotator_scalar_t<R>;
    ...
    return Q{ cr*sp*cy-sr*cp*sy, cr*cp*sy+sr*sp*cy, sr*cp*cy-cr*sp*sy, cr*cp*cy+sr*sp*sy };
}
/* ClampAxis: normalize degrees to [0, 360). NormalizeAxis: normalize to (-180, 180]. */
template<typename T> T ClampAxis   (T angle) noexcept { angle = std::fmod(angle,T(360)); if (angle<T(0)) angle+=T(360); return angle; }
template<typename T> T NormalizeAxis(T angle) noexcept { angle = ClampAxis(angle); if (angle>T(180)) angle-=T(360); return angle; }

template<LinearColor C, typename T = float> constexpr C LerpColor(const C& a, const C& b, T t) noexcept
{
